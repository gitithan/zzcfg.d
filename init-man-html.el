...
;;=========================================================
;;you have a lot of HTML documentation on your local system or URLs to remote resources. 
;;you want to use your favourite HTML browser on them; here¡¯s how:
...
;;   ("handbook" "/usr/share/doc/en_US.ISO_8859-1/books/handbook/index.html")
   ("lua" "E:/soft/lua/doc/contents.html")
...
;;the prompt defaults to the word the TextCursor sits on, 
;;but you can TAB-complete on each of the keys in the first column of the man-local-alist.
;;the URL in the second column will be given to your browser. 
;;think about adding a third column for a ¡°method¡± to call up other browsers 
;;(like for viewing pictures or simply using emacs M-x man command), 
;;or add a few lines to split the current window and put the docs in the upper one. also, 
;;it¡¯s not difficult to add a search string to the URL (or is it?).
;;
;;this code snippet used to be a (long) list of shell aliases, 
;;but for actual work i need an emacs with two windows: documentation in the upper and code in the lower one, so there!
;;
;;as the alist is easily extendible, i strongly recommend keeping it sorted. 
;;the above is heavily shortened  you can put local copies as well as remote URLs in there, 
;;just whatever your browser can understand when given as an argument.


(provide 'init-man-html)